...
echo "请输入开始查询的版本号（推荐从 40172 [B535] 开始）："
read v
echo "请输入结束查询的版本号:"
read e
...
Filter(){
    if [[ $1 == "1" ]]; then
        ...
        for f in `ls $PWD/TL00`; do
            File=`ls -il $PWD/TL00/$f | awk '{print $10}'`
            Size=`ls -il $PWD/TL00/$f | awk '{print $6}'`
...
update_url="http://update.hicloud.com:8180/TDS/data/files/p3/s15/G1021/g223/v$var/f1/full/update.zip"
